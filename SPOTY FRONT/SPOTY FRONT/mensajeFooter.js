...
function repetirCadaSegundo(){
    identificadorIntervaloDeTiempo = setTimeout(mandarMensaje, 12000);
}
function mandarMensaje(){
    let conf = confirm("¿Desea conocer mas musica?");
    if(conf){
        window.location.href = "https://open.spotify.com/";
    }else{
        const divP = document.querySelector('.perfil');
        divP.innerHTML += `<p class="parr">Usuario oficial de Spotifront</p>`
        const parr = document.querySelector('.parr');
        parr.style.color="green"
        parr.style.cursor="pointer"
