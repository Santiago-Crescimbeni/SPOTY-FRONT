...
// en este caso si el album tiene su propiedad like en true, se le agrega la clase "favorito" al elemento
...
const cantidadAlbum= albumesFamosos.length;
const classspan1= document.getElementById('cant-albums');   
classspan1.innerHTML= cantidadAlbum + `${cantidadAlbum > 1? ' albumes': ' album' }`


const classFavoritos= document.getElementById('cant-favoritos');  
let acumuladorMG=0;
albumesFamosos.forEach((megusta)=> {
   megusta.like == true? acumuladorMG+=1: null
})
classFavoritos.innerHTML= acumuladorMG + `${acumuladorMG >1? ' Favoritos': 'Favorito'}`;
